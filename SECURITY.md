...
- **A value typed into an app's Setup form is stored in the clear.** It is written
  ...
  and not a flaw in it. If you find a way to read one *without* physical access to
  the watch, though, that is very much in scope.
...
- **If I do not respond and it matters:** report it to GitHub. The binaries are
  ...
  repository without me. That route exists because the one below does not scale.
...
silently becoming something the catalogue cannot name. It takes effect on the next
catalogue build.
...
**It cannot take anything off a watch.** Kira never deletes an app folder, so
`settings.json` and `Activity/` survive an update, and it has no way to reach a
device that is not plugged in and driving the page. Removal stops *new* installs
and explains the situation to anyone who connects. F-Droid, Homebrew and crates.io
are all the same in this respect.
...
- Its source has disappeared, so the build can no longer be checked by anyone.
  That is grounds for retiring, not deleting: the binary is not suddenly unsafe,
  it has just stopped being verifiable.

## How quickly

This is one person's side project, and **nobody else holds the keys.** Something
obviously dangerous will be retired as soon as it is read, which is a matter of
hours or days depending entirely on what else is happening. Nothing here is on
call.

If that is not good enough for your situation, and for a genuinely malicious app
it should not be, use the GitHub route above instead of waiting on me.
...
respond at all. That is why nothing in Kira's design asks you to trust that I am
available.
